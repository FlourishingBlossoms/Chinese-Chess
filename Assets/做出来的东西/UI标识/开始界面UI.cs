...
public class 开始界面UI : MonoBehaviour
{
    ...
    public GameObject 模式选择界面;  // 新增：模式选择面板
    ...
    // ==================== 登录相关 ====================
    public LoginUIControl loginControl;
    public Button 创建房间;
    public Button 加入房间;
    public Button 返回;
// ==================== 房间操作相关 ==========

    public GameObject 房间操作面板;
    public GameObject 房间列表面板;
    public Transform 房间列表容器;
    public GameObject 房间列表项预制体;
    // ==================== 内部状态 ====================
    private bool isLoading = false;
    ...
    private void Start()
    {
        ...
        if (EXtra != null) EXtra.onClick.AddListener(EXtraGame);
        if (Practice != null) Practice.onClick.AddListener(PracticeGame);
        if (双人模式 != null) 双人模式.onClick.AddListener(DoubleGame);
    ...
    private void OnStartButtonClick()
    {
        Debug.Log("[开始界面UI] 点击开始");

        // 检查是否已登录
        if (LoginUIControl.CurrentUserId != -1)
        {
            // 已登录，直接显示模式选择
            Debug.Log("[开始界面UI] 已登录，显示模式选择界面");
            ShowOnlyPanel(模式选择界面);
        }
        else
        {
            // 未登录，显示登录界面
            Debug.Log("[开始界面UI] 未登录，显示登录界面");
            ShowOnlyPanel(登录界面);
        }
    }
    ...
    public void DoubleGame()  // 新增双人模式
    {
        Debug.Log("[开始界面UI] 选择双人模式");
        PlayerPrefs.SetString("GameMode", "Double");
        ShowOnlyPanel(双人);
    }
    ...
    public IEnumerator Loading(string sceneName)
    {
        ...
        if (EX != null) EX.SetActive(false);
        if (登录界面 != null) 登录界面.SetActive(false);
        if (模式选择界面 != null) 模式选择界面.SetActive(false);
